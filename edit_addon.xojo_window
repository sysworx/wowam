...
Begin Window edit_addon
   ...
   Begin Label lbl_desc
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Edit the Url from: AddonName"
      TextAlign       =   0
      TextColor       =   &c00000000
      TextFont        =   "System"
      TextSize        =   0.0
   ...
   Begin Label lbl_note
      ...
      TabPanelIndex   =   0
      TabStop         =   True
      Text            =   "Some developers call their addons on curse not the same as in the toc files. If this happens, delete the addon and add it again."
      TextAlign       =   0
      TextColor       =   &c00000000
      TextFont        =   "System"
      TextSize        =   0.0
...
#tag WindowCode
	#tag Property, Flags = &h0
		listboxAddon As String
	#tag EndProperty

	#tag Property, Flags = &h0
		listboxRow As Integer
	#tag EndProperty
		...
		Sub Action()
		  ...
		  if url.Len > 0 then
		    ...
		    if url.InStr("https://mods.curse.com/addons/wow") >0 then 
		      PreferencesModule.Log("Edit", "-------- Starting Edit Procedure for " + main.listbox1.Cell(listboxRow,0) + " -----------------")
		      'load the website
		      source=h.Get(url,50)
		      'search addon name
		      ...
		      'compare the names
		      if listboxAddon = AddonName then
		        'edit the listboxCell
		        main.listbox1.Cell(listboxRow,3) = txt_url.Text
		        listboxRow = -1
		        listboxAddon = ""
		        'send the data to sys-worx
		        senddata.Value("addon") = AddonName
		        senddata.Value("url") = txt_url.Text
		        ...
		        MsgBox "The URL seems not to be the same Addon as in the list!"
		      end if
